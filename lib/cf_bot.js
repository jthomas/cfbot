...
class CFBot {
  ...
  update_cf_tokens (tokens) {
    ...
    // refresh tokens an hour before they run out.
    var timeout = (tokens.expires_in - (60 * 60)) * 1000
    var token_refresh = this.refresh_cf_tokens.bind(this, this.update_cf_tokens.bind(this))
    setTimeout(token_refresh, timeout)
  ...
  on_cf_event (event) {
    ...
    if (messages && this.match_filter_set(this.filters.apps, event.actee_name)
